...
void gameover(string enermy_name,int pos,int tick){
	...
	if (battery<0){
		cout<<"\t\tYou've ran out of battery!"<<endl;
	}
...
bool enemyAttack(Enemy e) {
	if (battery < 0) return true; // Player is dead if battery is below 0
...
void showBattery(int battery,int usage){
	gotoxy(30, 4);
	cout << "Battery: " << battery/500 << "% | Usage: " << usage;
}
...
void play(int code){
	...
	// Giới hạn số vòng lặp mỗi giây (ví dụ: 40 vòng/giây)
	const int FPS = 60;
	...
	while(1){
		...
		showBattery(battery, battery_usage);
		if (tick%10==0){
		...
		if (battery <=0){
			iscomputerOn = false;
			isFanOn = false;
			computer_mode = 1; // turn off computer and fan
			battery_usage = 0; // reset battery usage
		}
		battery -= battery_usage; // decrease battery by battery usage
		 // reset battery usage
...
void instructions(){
	...
	cout<<"\n8. tip 3: manage your battery wisely, avoid using unnecessary power.";
